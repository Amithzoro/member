...
def member_management(members_df):
    ...
    edit_id = st.number_input("Enter Member ID to Edit", min_value=1, step=1)
    if st.button("Load Member"):
        if edit_id in members_df['ID'].values:
            member = members_df[members_df['ID']==edit_id].iloc[0]
            name_edit = st.text_input("Full Name", member['Name'], key=f"edit_name_{edit_id}")
            phone_edit = st.text_input("Phone", member['Phone'], key=f"edit_phone_{edit_id}")
            mtype_edit = st.selectbox("Membership Type", ['Monthly','Quarterly','Yearly'], index=['Monthly','Quarterly','Yearly'].index(member['Membership Type']), key=f"edit_mtype_{edit_id}")
            join_edit = st.date_input("Join Date", member['Join Date'].date(), key=f"edit_join_{edit_id}")
            ...
            if st.button("Save Changes", key=f"save_{edit_id}"):
                members_df.loc[members_df['ID']==edit_id, ['Name','Phone','Membership Type','Join Date','Expiry Date']] = [name_edit, phone_edit, mtype_edit, join_edit, expiry_edit]
                save_data(members_df)
                st.success(f"Member ID {edit_id} updated")
        else:
...
def reminders_popup(members_df):
    ...
    df = members_df.copy()
    df['Days Left'] = (df['Expiry Date'] - now).apply(lambda x: x.days)
    expiring = df[(df['Days Left'] >=0) & (df['Days Left'] <=30)]
    expired = df[df['Days Left'] < 0]
    if not expired.empty:
        st.warning(f"{len(expired)} members expired!")
        st.dataframe(expired)
    if not expiring.empty:
        st.info(f"{len(expiring)} members expiring in 30 days!")
