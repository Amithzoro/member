...
def load_database():
    ...
    # Ensure proper types
    member_df['Join Date'] = pd.to_datetime(member_df.get('Join Date', pd.Series([])), errors='coerce').dt.date
    member_df['Expiry Date'] = pd.to_datetime(member_df.get('Expiry Date', pd.Series([])), errors='coerce').dt.date
    member_df['ID'] = member_df.get('ID', pd.Series(dtype=int)).astype(int) if not member_df.empty else pd.Series(dtype=int)

    log_df['CheckIn Time_dt'] = pd.to_datetime(log_df.get('CheckIn Time', pd.Series([])), errors='coerce')
    return member_df, log_df

def save_database(member_df, log_df):
    member_df_safe = member_df.copy()
    member_df_safe['Join Date'] = member_df_safe['Join Date'].apply(lambda x: x if isinstance(x, datetime.date) else get_ist_time().date())
    member_df_safe['Expiry Date'] = member_df_safe['Expiry Date'].apply(lambda x: x if isinstance(x, datetime.date) else get_ist_time().date() + datetime.timedelta(days=30))

    with pd.ExcelWriter(DB_FILE, engine='openpyxl') as writer:
        member_df_safe.to_excel(writer, sheet_name='Members', index=False)
...
def check_in(member_df, log_df):
    ...
    if st.button("Record Entry"):
        ...
        if member.empty:
            ...
            if not isinstance(expiry, datetime.date) or expiry < today:
...
def reminders(member_df):
    ...
    today = get_ist_time().date()
    df = member_df.copy()
    df['Days Left'] = df['Expiry Date'].apply(lambda x: (x - today).days if isinstance(x, datetime.date) else None)
